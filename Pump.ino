...
if (cMillis - pMillis >= 100) {
  ...
  //水泵运行条件,在8~18点，00分，30秒起动，延时3s，如果流量<2L/min停机
  if ((hour()>=8) && (hour()<=18)  && (minute()==0) && (second()==30)){
    digitalWrite(9, LOW);
    pump_t=millis();
  }else{
    if (((millis()-pump_t>3000) && (flow_l<2)) || ((minute()==20) && (second()==30))){
    digitalWrite(9, HIGH);
    }
  }    
  //串口数据用于诊断↓
